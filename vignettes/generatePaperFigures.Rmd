...
K <- ceiling(runif(n_pops * length(n_iso_scalar), min = 3, max = 20 ))
...
# uncorrected X_var
g1a <- ggplot(data = iso_tbl %>% filter(n_iso == 1), 
  ...
  theme_classic() + 
  coord_fixed(ratio = 0.125/2, xlim = c(0, 0.125), ylim = c(0,2))

# r<-max(abs(layer_scales(g1a)$x$range$range))
# s<-max(abs(layer_scales(g1a)$y$range$range))
# t<-round(max(r,s),1)
# g1a <- g1a+coord_equal(xlim=c(0,t),ylim=c(0,t))

g1b <- ggplot(data = iso_tbl %>% filter(n_iso == 1), 
...
# plot the corrected data for 2 isotopes and then 10 isotopes
g1c <- g1b %+% (iso_tbl %>% filter(n_iso == 2))  + ggtitle("2 isotopes")
g1d <- g1b %+% (iso_tbl %>% filter(n_iso == 10)) + ggtitle("10 isotopes")

# print to screen in a one over three format as i cant square the axis of 
# panel 1.
# (g1a  / (g1b | g1c | g1d))

g1a + g1b + g1c + g1d + plot_layout(ncol = 2, nrow = 2, widths = c(1,1))

ggsave(plot = (g1a),
       paste0("../export/g1a.png"),
       width = 7, height = 7)

ggsave(plot = (g1b),
       paste0("../export/g1b.png"),
       width = 7, height = 7)

ggsave(plot = (g1c),
       paste0("../export/g1c.png"),
       width = 7, height = 7)

ggsave(plot = (g1d),
       paste0("../export/g1d.png"),
       width = 7, height = 7)
...
The correlations between panels a, b, c, d

```{r}

iso_tbl %>% group_by(n_iso) %>% 
  summarise(rho_uncorrected = cor(Y_var, p_trace),
            rho_corrected   = cor(Y_var / X_var, ((K) / (K-1)) * p_trace)) %>% 
  round(2)
...
g2 <- ggplot(iso_tbl %>% filter(n_iso == 1), 
             aes(x = K * p_aniso,
...
# (g2_abs + scale_x_log10() + scale_y_log10())
(g2_abs)

ggsave(g2_abs, 
       filename = paste0("../export/fig2_n_iso_1_anisotropy.png"), 
       width = 5, height = 5)
