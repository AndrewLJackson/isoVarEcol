...
Since we simulated our data with across independent random draws of $K$, $\sigma_{x^*}$ and $\sigma_f$, we can explore the relationships between the observed consumer isotope variation $\hat{\sigma}_x$ and the observed statistical approximations of these simulated processes. Specifically we are looking for correlations according to the following statistical model based on the analytical model presented.

$$\sum\text{cov}(x_k,x_K) \propto \sigma^2_{x^*} (K-1)  \sum_{i,j}\text{cov}(f_i,f_j)$$

There are three things to note from this parameterisation:

- whereas in the mathematical model, the dietary proportion $f_m$ defined as the source closest to the mean of the sources would be ommitted from the calculations, here assume we do not have access to this information and instead arbitrarily remove the first dietary proportion $f_1$ so that $i$ and $j$ range from 2 to $K$. 
- I have not decomposed the covariance in diet into variance and correlation coefficient.
- We also simulated over 2 isotopes in this case, and so the variation among consumers is calculated as the sum of the covariance over the $w \times K$ matrix of the consumer population's observed isotope values.

Since the empirical focus would be on using consumer variation as the most likely available data, we explore the pairwise correlations between single parameters on the right hand side of the above equation, with $\sum\text{cov}(x_k,x_K)$ after correcting for the remaining two variables by dividing by $\sigma^2_{x^*} (K-1)$ in the example case of expecting a 1:1 relationship between:

$$\frac{\sum\text{cov}(x_k,x_K)}{\sigma^2_{x^*} (K-1)} \propto \sum_{i,j}\text{cov}(f_i,f_j)$$


### Correlation between diet variation and consumer variation

Here `cov(p-1)` is the sum of the covariance matrix of the dietary proportions omitting the first source and `X_var` is the sum of the covariance of the consumer isotope matrix. This is the *main* result of interest.
...
**Figure 1.** The relationship between simulated consumer isotope variation (X_var = $\hat{\sigma}^2_x$) in **a**, and corrected isotope variation (X_var_corrected = $\frac{\hat{\sigma}^2_x}{\sigma^2_{x^*}(K-1)}$) in **b** against dietary variation ($\sum_{i,j}\text{cov}(f_i,f_j)$). The blue regression line in **a** is a linear fit, while the red line in **b** is the analytical prediction for a 1:1 scaling relationship.


### Coorelation between source variation and consumer variation

Here we explore the effect of variation among source isotope values (i.e. isotopic baselines in ecology parlance) on conumser isotope variation. Given the relative scales of the various parameters, this has the largest effect. 
...
**Figure 2.** The relationship between simulated consumer isotope variation (X_var = $\hat{\sigma}^2_x$) in **a**, and corrected isotope variation (X_var_corrected = $\frac{\hat{\sigma}^2_x}{\sigma^2_{x^*}(K-1)}$) in **b** against source isotope variation ($\sigma^2_{x^*}$). The blue regression line in **a** is a linear fit, while the red line in **b** is the analytical prediction for a 1:1 scaling relationship.


### Correlation between number of sources and consumer variation

Here we explore the expected 1:1 correlation between $(K-1)$ and $\sigma^2_x$.
...
# fit a linear model
m2 <- glm(log(X_var) ~ log(I(K-1)), data = iso_tbl)

g2 <- ggplot(data = iso_tbl, 
             mapping = aes(x = K-1, y = X_var)) + 
  geom_point() + 
  geom_smooth(method = "lm") +
...
**Figure 3.** The relationship between the observed consumer isotope variation $\hat{\sigma}^2_x$ and the marginal effect of source isotope variation $\sigma^2_{x^*}$  in **a** and the corrected   **b**. The blue line in **a** and **b** is a regression line fitted to the data, and the red line in **b** is the analytical predicted 1:1 relationship from the analytical model.
...
**Figure 3.** The relationship between the observed consumer isotope variation $\hat{\sigma}^2_x$ and the marginal effects of number of food sources $K-1$ in **a** and the corrected consumer isotope variation in **b**. The blue line in **a** and **b** is a regression line fitted to the data, and the red line in **b** is the analytical predicted relationship of $\hat{\sigma}^2_x = (K-1)$.
...
**_Again i am suprised by this plot. I was expecting almost perfect correlation with a slope = 1 and an intercept close to 0._**
