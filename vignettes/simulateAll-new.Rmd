...
# subsequent step.
iso_tbl %<>% mutate(S_max = pmap(list(n_iso), genSourceRange, max_val = 1))
...
iso_tbl %<>% 
  mutate(SEA     = pmap(list(X), ~doSEA(.)) %>% unlist,
         ...
         X_trace = pmap(list(X), ~sum(diag(cov(.)))) %>% unlist
         )
...
iso_tbl %<>% 
  mutate(S_var   = pmap(list(S), ~sum(cov(.))) %>% unlist,
         S_trace = pmap(list(S), ~sum(diag(cov(.)))) %>% unlist)
...
g_test_1 <- ggplot(data = iso_tbl, mapping = aes(x = X_jeff, X_var)) + 
  geom_point()


g_test_2 <- ggplot(data = iso_tbl, mapping = aes(x = S_var, S_trace)) + 
  geom_point()

(g_test_1 | g_test_2)

g_test_3 <- ggplot(data = iso_tbl, 
             mapping = aes(x = X_theo,
                           y = ((K) / (K-1)) * p_trace)) +
  geom_point()
...
(g1 | g1_corrected | g1_test)

ggsave(plot = (g1|g1_corrected | g1_test), filename = "consumer-correlates-with-diet.pdf", width = 10)
```
...
ggsave((g2 | g2_abs), filename = "anisotropy.pdf", width = 10)
