...
# Source isotopes range from 0 to source_iso_max and are uniform random.
source_iso_max <- 10
...
iso_tbl %<>% mutate(p_mean_sigma = map(p_eig_values, ~mean(.[-length(.)])) %>% unlist)
iso_tbl %<>% mutate(X_theo = K * p_mean_sigma)
...
# Calculate various measures of consumer variation
iso_tbl %<>% 
  mutate(SEA     = pmap(list(X), ~doSEA(.)) %>% unlist,
         X_var   = pmap(list(X), ~sum(apply(., 2, fooVar))) %>% unlist,
...
# Calculate various measures of source variation
iso_tbl %<>% 
  mutate(S_var   = pmap(list(S), ~sum(apply(., 2, popVar))) %>% unlist,
         S_trace = pmap(list(S), ~sum(diag(popVar(.))))     %>% unlist)
...
ggsave(plot = (g1),
       paste0("n_iso_", n_iso, "_naive_correlation.pdf"), 
       width = 7, height = 7)
ggsave(plot = (g1_corrected), 
       filename = paste0("n_iso_", n_iso, "_corrected_correlation.pdf"), 
       width = 7, height = 7)
```
...
g2 <- ggplot(iso_tbl, aes(x = K * p_aniso,
                          y = (X_var / S_var) - (K / (K-1)) * p_trace )) + 
  geom_point(alpha = 1, size = 0.5) +
  theme_classic()

# absolute deviations
g2_abs <- g2 + aes(y = abs((X_var / S_var) - (K / (K-1)) * p_trace )) + 
  geom_smooth(method = "lm", color = "black") + 
  ylab("Absolute Deviance") + 
  xlab(expression(italic(n)(max[alpha](sigma^2)[alpha]-min[alpha](sigma^2)[alpha])))

(g2_abs)

ggsave(g2_abs, 
       filename = paste0("n_iso_", n_iso, "_anisotropy.pdf"), 
...
m5 <- glm(log(X_var) ~ log(I(K/(K-1))) + log(S_var) + log(p_trace), data = iso_tbl)
...
ggplot(iso_tbl, aes(x = S_var, y = S_trace)) + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1, color = "red") + 
  geom_smooth(method = "lm")
